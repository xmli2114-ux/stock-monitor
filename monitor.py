...
def get_realtime_quote(code):
    ...
    try:
        return {"name": parts[0], "open": float(parts[1]) if parts[1] else 0,
...
{msg}"}}
    try:
        ...
        print("[OK] DingTalk pushed")
    except Exception as e: print(f"[FAIL] DingTalk: {e}")
...
def check_stock(code):
    try:
        ...
        if df is None or len(df) < 20: print(f"  [{name}] K-line data insufficient"); return None
        ...
        print(f"  [{name}] price={current:.2f} rise={rise_pct:.2f}% vol_ratio={vol_ratio:.2f}x ma20={close_ma20:.2f} dist_high={dist_from_high:.1f}%")
